...
void Renderer::InitGL()
{
    ...
    ASSERT( hasMultiTexture, "Multitexture not supported!" );

    bool hasTextureCombined  = glewGetExtension("GL_EXT_texture_env_combine");
    ASSERT( hasTextureCombined, "Texture environment combined not supported!" );

    int numTexUnits;
    glGetIntegerv( GL_MAX_TEXTURE_UNITS, &numTexUnits );
//    INFO( "Info", "Number of texture units %d", numTexUnits );
...
void Renderer::Run()
{
    ...
    try {
        ...
        ShowError( "Renderer failed with unknown Exception!", "Unknown Error!" );
        SendTerminate();
    }
}
