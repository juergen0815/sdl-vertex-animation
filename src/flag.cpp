...
//    glColor4f( 0.0f, 0.4f, 1.0f, 0.8f );
    ...
    if ( m_Texture) {
        m_Texture->Enable();
        float *texCoords = (float*)&m_TexCoordBuffer[0];
        // only use u/v coords, skip z/w - stride is from n[0] + offset = n[1]
        glTexCoordPointer( 2, GL_FLOAT, sizeof(Vector), texCoords );
    }
...
void Flag::DoUpdate( float ticks ) throw(std::exception)
{
    ...
    if ( m_TimeEllapsed*m_Speed > 16.67 )
    {
        ...
        auto& last = m_VertexBuffer[ m_VertexBuffer.size()-1];
